...
`ui://pawlens/hello-widget-v4.html` の返却リソースには `_meta.ui.csp` を設定しています。`connectDomains` は空の許可リストです。React の JavaScript とCSSはWorkerの `/assets/` から読むため、`resourceDomains` にはWorker自身の正確なHTTPSオリジンだけを登録しています。`pkgs/widget/public/_headers` は `/assets/*` にCORSヘッダーを付け、ChatGPTの別オリジンのサンドボックスからモジュールを安全に読めるようにします。外部通信やCDNを追加する場合だけ、その正確なHTTPSオリジンを対応するリストへ追加してください。`frameDomains` は不要なため設定しません。
...
3. **Resources** で `ui://pawlens/hello-widget-v4.html` を確認する。
...
2. ウィジェットでプロフィールを登録し、観察欄に次のように入力します。
...
3. 姿勢や玄関との距離が分かる写真を ChatGPT に添付済みなら、ウィジェットの **ChatGPT内の写真** から選択します。写真を再添付する必要はありません。ファイルライブラリを利用できない環境では、ウィジェット内の **写真を追加** を使います。

4. 見立て結果で、可能性・限界・次に観察すること・安全な次の一手を確認します。**ChatGPTで続きを相談する** を選択して、次の内容を続けて送ります。
...
5. 記録する場合は、結果に表示された観察ポイントのうち、飼い主が実際に確認できた事実だけを選択して保存します。AIの推測を確認済み事実として保存しません。
...
- **ウィジェットが出ない**: `pnpm run build` を先に実行し、`show_pawlens_hello` の呼び出しと `ui://pawlens/hello-widget-v4.html` のリソースを Inspector で確認します。
