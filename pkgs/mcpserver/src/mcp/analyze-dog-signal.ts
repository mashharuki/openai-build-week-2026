...
export function registerAnalyzeDogSignal(
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      // `locale` is part of the tool contract: it specifies the language for
      // owner guidance. It takes precedence over the ChatGPT UI locale so an
      // English request in a Japanese ChatGPT client remains all English.
